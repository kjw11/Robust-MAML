...
def train(model, saver, sess, str_dict, train_file_list, resume_itr=0):
    ...
    for itr in range(resume_itr, FLAGS.train_iterations):
        # Sample meta train and meta test in iterator lists
        ...
        for i in range(num_meta_train):
            ...
            if i == 0:
                inputa0, labela0, namea0 = sess.run(train_next_list[task_ind])
            else:
        ...
        for i in range(num_meta_test):
            ...
            if i == 0:
                inputb, labelb, nameb = sess.run(train_next_list[task_ind])
            else:
