...
class MASF:
    ...
    def construct_model_train(self, prefix='metatrain_'):
        ...
        self.inputb0 = tf.placeholder(tf.float32)
        self.inputb1 = tf.placeholder(tf.float32)
        self.labelb0 = tf.placeholder(tf.float32)
        self.labelb1 = tf.placeholder(tf.float32)
        ...
        with tf.variable_scope('model', reuse=None) as training_scope:
            ...
            def task_metalearn(inp, reuse=True):
                ...
                inputa0, labela0, inputb0, labelb0, inputa1, labela1, inputb1, labelb1 = inp
                ...
                # update grad based on genre_a_train
                ...
                fast_weights = dict(zip(weights.keys(), [weights[key] - self.inner_lr * tf.clip_by_norm(gradients[key], 
                                                                             clip_norm=self.clip_value) for key in weights.keys()]))
                # compute loss based on genre_a_test
                _, task_outputa1 = self.forward(inputa1, fast_weights, reuse=reuse, is_training=True)
                task_outputa1, task_lossa1 = self.loss_func(task_outputa1, labela1, is_training=True, reuse_variables=tf.AUTO_REUSE)


                # For genre b
                _, task_outputb0 = self.forward(inputb0, weights, reuse=reuse, is_training=True)
                task_outputb0, task_lossb0 = self.loss_func(task_outputb0, labelb0, is_training=True, reuse_variables=tf.AUTO_REUSE)
                # update grad based on genre_b_train
                grads = tf.gradients(task_lossb0, list(weights.values()))
                grads = [tf.stop_gradient(grad) for grad in grads] # first-order gradients approximation
                gradients = dict(zip(weights.keys(), grads))
                fast_weights = dict(zip(weights.keys(), [weights[key] - self.inner_lr * tf.clip_by_norm(gradients[key], 
                                                                             clip_norm=self.clip_value) for key in weights.keys()]))
                # compute loss based on genre_b_test
                _, task_outputb1 = self.forward(inputb1, fast_weights, reuse=reuse, is_training=True)
                task_outputb1, task_lossb1 = self.loss_func(task_outputb1, labelb1, is_training=True, reuse_variables=tf.AUTO_REUSE)

                
                #task_output = [global_loss, task_lossa, task_lossa1, metric_loss]
                task_output = [task_lossa1, task_lossb1]
                ...
                task_accuracya1 = tf.contrib.metrics.accuracy(tf.argmax(tf.nn.softmax(task_outputa1), 1), tf.argmax(labela1, 1)) #this accuracy already gathers batch size
                task_accuracyb0 = tf.contrib.metrics.accuracy(tf.argmax(tf.nn.softmax(task_outputb0), 1), tf.argmax(labelb0, 1)) #this accuracy already gathers batch size
                task_accuracyb1 = tf.contrib.metrics.accuracy(tf.argmax(tf.nn.softmax(task_outputb1), 1), tf.argmax(labelb1, 1)) #this accuracy already gathers batch size
                task_output.extend([task_accuracya0, task_accuracyb0, task_accuracya1, task_accuracyb1])
            ...
            input_tensors = (self.inputa0, self.labela0, 
                             self.inputb0, self.labelb0,
                             self.inputa1, self.labela1,
                             self.inputb1, self.labelb1)
        ...
        if 'train' in prefix:
            self.lossa = tf.reduce_mean(self.lossa_raw)
            self.lossb = tf.reduce_mean(self.lossb_raw)
            self.meta_loss = self.lossa + self.lossb
            ...
            self.accuracya0 = accuracya0 * 100.
            self.accuracya1 = accuracya1 * 100.
            self.accuracyb0 = accuracyb0 * 100.
            self.accuracyb1 = accuracyb1 * 100.

        ## Summaries
        tf.summary.scalar(prefix+'genre 1 meta test loss', self.lossa)
        tf.summary.scalar(prefix+'genre 2 meta test loss', self.lossb)
        tf.summary.scalar(prefix+'genre 1 meta train acc', self.accuracya0)
        tf.summary.scalar(prefix+'genre 1 meta test acc', self.accuracya1)
        tf.summary.scalar(prefix+'genre 2 meta train acc', self.accuracyb0)
        tf.summary.scalar(prefix+'genre 2 meta test acc', self.accuracyb1)
